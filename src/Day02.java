...
public class Day02 {
    ...
    private void doPart2(String input1) {
        ...
        long total = games.stream()
            .mapToLong(game-> getPowerOfSmallestNumberOfCubes(game))
            .sum();
    ...
    private Game parseGame(String line) {
        ...
        while (sc.hasNext()) {
            String set = sc.next().trim();
            String[] cubeRolls = set.split(",");
            for (String roll : cubeRolls) {
                String[] rollParts = roll.trim().split(" ");
                int cubeAmount = Integer.parseInt(rollParts[0]);
                String cubeColor = rollParts[1];
                rolls.add(new Roll(cubeColor, cubeAmount));
            }
        }
        return new Game(gameId, rolls);
    }

    private boolean gameIsValid(Game g){
        Map<String, Integer> cubeLimits = Map.of(
            "red", 12,
            "green", 13,
            "blue", 14
        );
        for (Roll roll : g.rolls) {
            if(roll.amount > cubeLimits.get(roll.color)){
                return false;
            }
        }
    ...
    private long getPowerOfSmallestNumberOfCubes(Game game) {
        ...
        for (Roll roll : game.rolls) {
            if (roll.amount > minCubes.get(roll.color)) {
                minCubes.put(roll.color, roll.amount);
            }
        }
        return ((long) minCubes.get("red")) * minCubes.get("green") * minCubes.get("blue");
    }

    private record Roll(String color, int amount){}
    private record Game(int gameId, List<Roll> rolls){}
