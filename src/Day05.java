...
public class Day05 {
    ...
    private void doPart1(String input1) {
        ...
        List<Long> seeds = parseSeeds(sc);
        List<Map> maps = parseMaps(sc);

        long lowestSeedStart = -1;
        long lowestSeedFinish = Long.MAX_VALUE;
        for (long seed : seeds) {
            long finish = traverseMaps(seed, maps);
            if (finish < lowestSeedFinish) {
                lowestSeedFinish = finish;
                lowestSeedStart = seed;
            }
        }
        System.out.println("Part1: " + lowestSeedStart + " -> " + lowestSeedFinish);
    }
    ...
    private void doPart2(String input1) {
        ...
        for (int i = 0; i < seeds.size(); i += 2) {
            long seedStartRange = seeds.get(i);
            long seedRangeLength = seeds.get(i + 1);
            long rangeEnd = seedStartRange + seedRangeLength;
            System.out.printf("Checking %,2d seeds \n", seedRangeLength);
            for (long j = seedStartRange; j < rangeEnd; j++) {
                long finish = traverseMaps(j, maps);
                if (finish < lowestSeedFinish) {
                    lowestSeedFinish = finish;
                    lowestSeedStart = j;
                }
        ...
        System.out.println("Part2: " + lowestSeedStart + " -> " + lowestSeedFinish);
    }

    private List<Map> parseMaps(Scanner sc) {
    ...
    private List<Long> parseSeeds(Scanner sc) {
        String seedsLine = sc.nextLine();
        seedsLine = seedsLine.substring(7);
        List<Long> seeds = new ArrayList<>();
        for (String s : seedsLine.split(" ")) {
            seeds.add(Long.parseLong(s));
        }
        return seeds;
    }

    private Map parseMap(Scanner sc) {
        ...
        while (sc.hasNext("\\d+")) {
            ...
            entries.add(new MapEntry(destinationRangeStart, sourceRangeStart, rangeLength));
        }
        return new Map(entries);
    }

    private long traverseMaps(long startingPoint, List<Map> maps) {
        for (Map map : maps) {
            startingPoint = traverseMap(startingPoint, map);
        }
        return startingPoint;
    }

    private long traverseMap(long startingPoint, Map map) {
        for (MapEntry mapEntry : map.mapEntries) {
            if (mapEntry.includedInRange(startingPoint)) {
                return mapEntry.map(startingPoint);
            }
        }
        return startingPoint;
    }
    ...
    private record MapEntry(long destinationRangeStart, long sourceRangeStart, long rangeLength) {
        public boolean includedInRange(long point) {
            return point >= sourceRangeStart && point < sourceRangeStart + rangeLength;
        }

        public long map(long point) {
            long adjustment = destinationRangeStart - sourceRangeStart;
            return point + adjustment;
        }
    }

    private record Map(List<MapEntry> mapEntries) {
    }
