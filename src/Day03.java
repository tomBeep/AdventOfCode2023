...
public class Day03 {
    ...
    private void doPart1(String input) {
        ...
        List<PartNumber> parts = findValidParts(grid);

        long sum = parts.stream().mapToLong(p -> Long.parseLong(p.partNumber)).sum();
    ...
    private void doPart2(String input) {
        ...
        for (int y = 0; y < grid.length; y++) {
            for (int x = 0; x < grid[y].length; x++) {
                char spot = grid[y][x];
                if (spot == '*') {
                    List<PartNumber> adjacentParts = getAdjacentValidParts(y, x, parts);
                    if (adjacentParts.size() == 2) {
                        long part1Number = Long.parseLong(adjacentParts.get(0).partNumber);
                        long part2Number = Long.parseLong(adjacentParts.get(1).partNumber);
                        sum += part1Number * part2Number;
                    }
                }
            }
        }
    ...
    private List<PartNumber> findValidParts(char[][] grid) {
        ...
        for (int y = 0; y < grid.length; y++) {
            for (int x = 0; x < grid[y].length; x++) {
                char characterAtSpot = grid[y][x];
                if (characterAtSpot >= '0' && characterAtSpot <= '9') {
                    partNumber += characterAtSpot;
                    partXEnd = x;
                    if (partXStart == -1) {
                        partXStart = x;
                    }
                } else {
                    // character at spot was not a digit, therefore we have reached the end of our partNumber.
                    if (partXStart != -1) {
                        PartNumber part = evaluatePartNumber(partNumber, y, partXStart, partXEnd, grid);
                        parts.add(part);
                    }
                    partNumber = "";
                    partXStart = -1;
            ...
            // We also need to reset at the end of each row...
            if (partXStart != -1) {
                PartNumber part = evaluatePartNumber(partNumber, y, partXStart, partXEnd, grid);
                parts.add(part);
            }
        ...
        return parts.stream().filter(p -> p.isValid).toList();
    }

    private PartNumber evaluatePartNumber(String partNumber, int y, int partNumberIndexStart, int partNumberIndexEnd, char[][] grid) {
        for (int i = partNumberIndexStart; i <= partNumberIndexEnd; i++) {
            if (hasAdjacentSymbol(y, i, grid)) {
                return new PartNumber(partNumber, y, partNumberIndexStart, partNumberIndexEnd, true);
            }
        }
        return new PartNumber(partNumber, y, partNumberIndexStart, partNumberIndexEnd, false);
    }

    private boolean hasAdjacentSymbol(int y, int x, char[][] grid) {
        return isSymbolGridSpot(y, x + 1, grid)
                || isSymbolGridSpot(y, x - 1, grid)
                || isSymbolGridSpot(y + 1, x, grid)
                || isSymbolGridSpot(y + 1, x + 1, grid)
                || isSymbolGridSpot(y + 1, x - 1, grid)
                || isSymbolGridSpot(y - 1, x, grid)
                || isSymbolGridSpot(y - 1, x + 1, grid)
                || isSymbolGridSpot(y - 1, x - 1, grid);
    ...
    private List<PartNumber> getAdjacentValidParts(int y, int x, List<PartNumber> validParts) {
        List<PartNumber> adjacentParts = new ArrayList<>();
        for (PartNumber part : validParts) {
            if (part.y == y || part.y == y + 1 || part.y == y - 1) {
                if (x >= part.xStart && x <= part.xEnd
                        || x + 1 >= part.xStart && x + 1 <= part.xEnd
                        || x - 1 >= part.xStart && x - 1 <= part.xEnd) {
    ...
    private boolean isSymbolGridSpot(int y, int x, char[][] grid) {
    ...
    private char[][] parseGrid(String input1) {
    ...
    private record PartNumber(String partNumber, int y, int xStart, int xEnd, boolean isValid) {
    }
